 ...
 */
namespace Auke\Ripcord\Server;

use Ripcord\Components\Ripcord;
...
class Ripcord_Server 
{
	...
	public function run() 
	{
		...
		if ( !$request_xml ) 
		{
			...
			{
				// XXX: add check for json-rpc protocol, if set and none of the xml protocols are set, use that
				header('Content-type: text/xml');
				header('Access-Control-Allow-Origin: *');
		...
		{
			// XXX: add check for the protocol of the request, could be json-rpc, then check if it is supported.
			header('Content-type: text/xml');
			header('Access-Control-Allow-Origin: *');
